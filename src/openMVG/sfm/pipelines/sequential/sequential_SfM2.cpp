  ...
  #pragma omp single nowait
#endif
    {
      ...
      if (!track_id_for_resection.empty() && track_ratio > track_inlier_ratio)
      {
		...
		//对得到的3D点进行重心化后计算得到pose
		Vec3 structure_centroid = Vec3::Zero();
		for (int i=0;i<resection_data.pt3D.cols();i++){
				structure_centroid += (resection_data.pt3D.col(i)/ (double)resection_data.pt3D.cols());
		}

		openMVG::geometry::Similarity3 sim_to_center = openMVG::geometry::Similarity3(openMVG::sfm::Pose3(Mat3::Identity(), structure_centroid), 1.0);
		// Transform the landmark positions
		for (int i = 0; i<resection_data.pt3D.cols(); i++){
			resection_data.pt3D.col(i) = sim_to_center(resection_data.pt3D.col(i));
		}
        ...
        if (bResection && inlier_ratio > 0.5)
        {
              ...
              resection_data, b_refine_pose, b_refine_intrinsics))
          {
			...
			//BA结束后再将pose反重心化
			openMVG::geometry::Similarity3 sim_inverse = sim_to_center.inverse();
			sfm_data_.poses[view->id_pose] = sim_inverse(pose);
            //Verified
          }
        }
      }
...
bool SequentialSfMReconstructionEngine2::BundleAdjustment()
{
  ...
  const Optimize_Options ba_refine_options
    ( ReconstructionEngine::intrinsic_refinement_options_,
      ...
      0//no need to use rigid transformations repeatedly
    );
