...
template <typename CostFunctor>
struct WeightedCostFunction_group
{
	WeightedCostFunction_group() : weight_(1.0) {}

	explicit WeightedCostFunction_group
	(
		CostFunctor * func,
		const double weight
	) :
	...
	{
		if (functor_->operator()(cam_intrinsic,cam_calibration, cam_extrinsics, pos_3dpoint, out_residuals))
		{
			// Reweight the residual values
			for (int i = 0; i < CostFunctor::num_residuals(); ++i)
			{
	...
	{
		if (functor_->operator()(cam_intrinsic, cam_calibration, cam_sevenparameters, cam_extrinsics, pos_3dpoint, out_residuals))
		{
			// Reweight the residual values
			for (int i = 0; i < CostFunctor::num_residuals(); ++i)
			{
