...
struct Control_Point_Parameter
{
	...
	(
		...
		bool use_multi_coordinate = false
  ): weight(weight_val), bUse_control_points(use_control_points),bUse_multi_coordinate(use_multi_coordinate)
  {}
  double weight;
  bool bUse_control_points;
  bool bUse_multi_coordinate;
};
...
struct Optimize_Options
{
  ...
  (
    ...
    const bool use_motion_priors = false,const bool use_sim = true,int cam_id=0,bool isGrouped=false
  )
